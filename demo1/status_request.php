						...
						<table id="add1" class="display table table-striped table-hover">
							...
							<tbody>
								...
								while ($data = mysqli_fetch_array($query, MYSQLI_BOTH)) {
									...
									if ($status == "1") {
										$status = "<b style='color:green'>ACC data lengkap</b>";
									} elseif ($status == "0") {
										$status = "<b style='color:red'>sedang diperiksa</b>";
									} elseif ($status == "2") {
										$status = "<b style='color:blue'>ACC surat permohonan</b>";
									} elseif ($status == "3") {
										$status = "<b style='color:green'>Permohonan selesai</b>";
									}
								?>
									<tr>
										...
										<td>
											<div class="form-button-action">
												...
												<?php if ($data['status'] == 2): ?>
													<a href="?halaman=view_cetak_user&id_request_sktm=<?= $id_request_sktm; ?>"
														class="btn btn-primary btn-sm"
														data-toggle="tooltip"
														title="Cetak Surat">
														Cetak Surat
									...
									<tr>
										...
										<td>
											<div class="form-button-action">
												...
												<!-- Print Letter Button -->
												<?php if ($data['status'] == 2): ?>
													<a href="?halaman=view_cetak_sku_user&id_request_sku=<?= $id_request_sku; ?>"
														class="btn btn-primary btn-sm"
														data-toggle="tooltip"
														title="Cetak Surat">
														Cetak Surat
									...
									<tr>
										...
										<td>
											<div class="form-button-action">
												...
												<!-- Print Letter Button -->
												<?php if ($data['status'] == 2): ?>
													<a href="?halaman=view_cetak_skp_user&id_request_skp=<?= $id_request_skp; ?>"
														class="btn btn-primary btn-sm"
														data-toggle="tooltip"
														title="Cetak Surat">
														Cetak Surat
									...
									<tr>
										...
										<td>
											<div class="form-button-action">
												...
												<!-- Print Letter Button -->
												<?php if ($data['status'] == 2): ?>
													<a href="?halaman=view_cetak_skd_user&id_request_skd=<?= $id_request_skd; ?>"
														class="btn btn-primary btn-sm"
														data-toggle="tooltip"
														title="Cetak Surat">
														Cetak Surat
...
if (isset($_GET['id_request_skd'])) {
	$hapus = mysqli_query($konek1, "DELETE FROM data_request_skd WHERE id_request_skd=$id_request_skd");
	if ($hapus) {
	...
	$hapus = mysqli_query($konek1, "DELETE FROM data_request_sktm WHERE id_request_sktm=$id_request_sktm");
	if ($hapus) {
		echo "<script language='javascript'>swal('Selamat...', 'Hapus Berhasil', 'success');</script>";
		echo '<meta http-equiv="refresh" content="3; url=?halaman=tampil_status">';
	} else {
	...
	$hapus = mysqli_query($konek1, "DELETE FROM data_request_skp WHERE id_request_skp=$id_request_skp");
	if ($hapus) {
		echo "<script language='javascript'>swal('Selamat...', 'Hapus Berhasil', 'success');</script>";
		echo '<meta http-equiv="refresh" content="3; url=?halaman=tampil_status">';
	} else {
	...
	$hapus = mysqli_query($konek1, "DELETE FROM data_request_sku WHERE id_request_sku=$id_request_sku");
	if ($hapus) {
		echo "<script language='javascript'>swal('Selamat...', 'Hapus Berhasil', 'success');</script>";
		echo '<meta http-equiv="refresh" content="3; url=?halaman=tampil_status">';
	} else {
